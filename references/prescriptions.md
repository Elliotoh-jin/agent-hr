...
| The failure-detector lives inside the thing it monitors | Internal audit reporting to the department it audits | Move the detector out of the loop: a separate process that reads artifacts, never the loop's own self-report. A loop that must run to announce its own death announces nothing once dead — and the detector itself needs a liveness signal, or it fails the same way |
| Absence in your window read as absence of work | HQ counts only what its dashboard collects; branch activity the metric can't see is filed as "no output" | Register the observation window per loop: record where each output lands, and mark outputs landing outside the monitored store as human-verified rather than missing. An unregistered window silently becomes the definition of reality |
